SELECT
  F4211.SDMCU F4211_SDMCU,
  F4211.SDDOCO F4211_SDDOCO,
  F4211.SDDCTO F4211_SDDCTO,
  F4211.SDADDJ F4211_SDADDJ,
  F0101.ABAN83 F0101_ABAN83,
  F4211.SDSHAN F4211_SDSHAN,
  F0101.ABALPH F0101_ABALPH,
  C.ALCTY1 F0116_ALCTY1,
  F4211.SDLITM F4211_SDLITM,
  F4211.SDSHPN F4211_SDSHPN,
  F4211.SDSRP1 F4211_SDSRP1,
  F4211.SDCARS F4211_SDCARS,
  F4211.SDMOT F4211_SDMOT,
  F4211.SDFRTH F4211_SDFRTH,
  F4211.SDURRF F4211_SDURRF,
  F4211.SDUOM F4211_SDUOM,
  F4211.SDLTTR F4211_SDLTTR,
  F4211.SDNXTR F4211_SDNXTR,
  CAST(F4211.SDUPRC AS FLOAT) / 1000000 F4211_SDUPRC,
  (126191 - F4211.SDADDJ) ID_CUSTOM_8d898e8eab5e711,
  SUM(F4211.SDAEXP) ReportColumn1,
  SUM(F4211.SDUORG) ReportColumn3,
  SUM(F4211.SDPQOR) ReportColumn4,
  F0010.CCCRCD DomesticCurrency
FROM
  PRODDTA.F4211 F4211
  INNER JOIN (
    SELECT
      F0101.ABAN83,
      F0101.ABALPH,
      F0101.ABSIC,
      F0101.ABAN8
    FROM
      PRODDTA.F0101 F0101
    WHERE
      (
        (F0101.ABAT1 BETWEEN N'A  ' AND N'P  ')
        OR (F0101.ABAT1 BETWEEN N'R  ' AND N'ZZZ')
      )
  ) F0101 ON F0101.ABAN8 = F4211.SDSHAN
  INNER JOIN PRODDTA.F0116 C
  INNER JOIN (
    SELECT
      F0116.ALAN8,
      MAX(F0116.ALEFTB) ALEFTB
    FROM
      PRODDTA.F0116 F0116
    GROUP BY
      F0116.ALAN8
  ) D ON (C.ALAN8 = D.ALAN8)
  AND (C.ALEFTB = D.ALEFTB) ON F0101.ABAN8 = D.ALAN8
  INNER JOIN PRODDTA.F0010 F0010 ON F0010.CCCO = F4211.SDKCOO
WHERE
  (
    (
      (
        (
          (
            ((F4211.SDDCTO IN (N'SO', N'CO')))
            AND (
              (
                (
                  NVL (RTRIM (LTRIM (F4211.SDMCU)), N' ') LIKE N'%97%'
                )
                OR (
                  NVL (RTRIM (LTRIM (F4211.SDMCU)), N' ') LIKE N'%98%'
                )
              )
            )
          )
        )
        AND (
          (
            F0101.ABSIC IN (N'F         ', N'FA        ', N'FB        ')
          )
        )
      )
    )
    AND ((126191 - F4211.SDADDJ) < 91)
  )
GROUP BY
  F4211.SDMCU,
  F4211.SDDOCO,
  F4211.SDDCTO,
  F4211.SDADDJ,
  F0101.ABAN83,
  F4211.SDSHAN,
  F0101.ABALPH,
  C.ALCTY1,
  F4211.SDLITM,
  F4211.SDSHPN,
  F4211.SDSRP1,
  F4211.SDCARS,
  F4211.SDMOT,
  F4211.SDFRTH,
  F4211.SDURRF,
  F4211.SDUOM,
  F4211.SDLTTR,
  F4211.SDNXTR,
  CAST(F4211.SDUPRC AS FLOAT) / 1000000,
  (126191 - F4211.SDADDJ),
  F0010.CCCRCD
UNION ALL
SELECT
  F4211.SDMCU F4211_SDMCU,
  F4211.SDDOCO F4211_SDDOCO,
  F4211.SDDCTO F4211_SDDCTO,
  F4211.SDADDJ F4211_SDADDJ,
  F0101.ABAN83 F0101_ABAN83,
  F4211.SDSHAN F4211_SDSHAN,
  F0101.ABALPH F0101_ABALPH,
  C.ALCTY1 F0116_ALCTY1,
  F4211.SDLITM F4211_SDLITM,
  F4211.SDSHPN F4211_SDSHPN,
  F4211.SDSRP1 F4211_SDSRP1,
  F4211.SDCARS F4211_SDCARS,
  F4211.SDMOT F4211_SDMOT,
  F4211.SDFRTH F4211_SDFRTH,
  F4211.SDURRF F4211_SDURRF,
  F4211.SDUOM F4211_SDUOM,
  F4211.SDLTTR F4211_SDLTTR,
  F4211.SDNXTR F4211_SDNXTR,
  CAST(F4211.SDUPRC AS FLOAT) / 1000000 F4211_SDUPRC,
  (126191 - F4211.SDADDJ) ID_CUSTOM_8d898e8eab5e711,
  SUM(F4211.SDAEXP) ReportColumn1,
  SUM(F4211.SDUORG) ReportColumn3,
  SUM(F4211.SDPQOR) ReportColumn4,
  F0010.CCCRCD DomesticCurrency
FROM
  PRODDTA.F42119 F4211
  INNER JOIN (
    SELECT
      F0101.ABAN83,
      F0101.ABALPH,
      F0101.ABSIC,
      F0101.ABAN8
    FROM
      PRODDTA.F0101 F0101
    WHERE
      (
        (F0101.ABAT1 BETWEEN N'A  ' AND N'P  ')
        OR (F0101.ABAT1 BETWEEN N'R  ' AND N'ZZZ')
      )
  ) F0101 ON F0101.ABAN8 = F4211.SDSHAN
  INNER JOIN PRODDTA.F0116 C
  INNER JOIN (
    SELECT
      F0116.ALAN8,
      MAX(F0116.ALEFTB) ALEFTB
    FROM
      PRODDTA.F0116 F0116
    GROUP BY
      F0116.ALAN8
  ) D ON (C.ALAN8 = D.ALAN8)
  AND (C.ALEFTB = D.ALEFTB) ON F0101.ABAN8 = D.ALAN8
  INNER JOIN PRODDTA.F0010 F0010 ON F0010.CCCO = F4211.SDKCOO
WHERE
  (
    (
      (
        (
          (
            ((F4211.SDDCTO IN (N'SO', N'CO')))
            AND (
              (
                (
                  NVL (RTRIM (LTRIM (F4211.SDMCU)), N' ') LIKE N'%97%'
                )
                OR (
                  NVL (RTRIM (LTRIM (F4211.SDMCU)), N' ') LIKE N'%98%'
                )
              )
            )
          )
        )
        AND (
          (
            F0101.ABSIC IN (N'F         ', N'FA        ', N'FB        ')
          )
        )
      )
    )
    AND ((126191 - F4211.SDADDJ) < 91)
  )
GROUP BY
  F4211.SDMCU,
  F4211.SDDOCO,
  F4211.SDDCTO,
  F4211.SDADDJ,
  F0101.ABAN83,
  F4211.SDSHAN,
  F0101.ABALPH,
  C.ALCTY1,
  F4211.SDLITM,
  F4211.SDSHPN,
  F4211.SDSRP1,
  F4211.SDCARS,
  F4211.SDMOT,
  F4211.SDFRTH,
  F4211.SDURRF,
  F4211.SDUOM,
  F4211.SDLTTR,
  F4211.SDNXTR,
  CAST(F4211.SDUPRC AS FLOAT) / 1000000,
  (126191 - F4211.SDADDJ),
  F0010.CCCRCD
ORDER BY
  F4211_SDDOCO ASC,
  F4211_SDLITM ASC,
  F4211_SDURRF ASC,
  F4211_SDDCTO ASC,
  F4211_SDMOT ASC,
  F0101_ABALPH ASC,
  F4211_SDLTTR ASC,
  F4211_SDNXTR ASC,
  F0101_ABAN83 ASC,
  F4211_SDFRTH ASC,
  F4211_SDUOM ASC,
  F4211_SDSRP1 ASC,
  F4211_SDUPRC ASC,
  F4211_SDMCU ASC,
  F4211_SDADDJ ASC,
  F4211_SDSHAN ASC,
  F0116_ALCTY1 ASC,
  F4211_SDSHPN ASC,
  F4211_SDCARS ASC,
  ID_CUSTOM_8d898e8eab5e711 ASC
...
TABLES = {
    ...
    'fact_price_adjustment': {
      ...
      "measures": [
        ...
        # product-line tons only (excludes F/FT freight lines) — identical to Total Tons on product lines, blank on
        # freight lines. Bind the SOP620 bucket-report Total Tons column here so freight rows show blank like Hubble.
        ('Total Tons (Product)', "SUMX(FILTER('fact_sales_order_freight', NOT(TRIM('fact_sales_order_freight'[line_type]) = \"F\" || TRIM('fact_sales_order_freight'[line_type]) = \"FT\")), 'fact_sales_order_freight'[transaction_quantity] * COALESCE(IF(TRIM('fact_sales_order_freight'[uom]) = \"TN\", 1.0), RELATED(dim_uom_conversion_item[conv_factor]), RELATED(dim_uom_conversion[std_factor]), 0))", '#,0.00', 'Price Adjustment'),
        ...
        # product-line extended_price only (excludes F/FT freight lines, whose amount belongs in Freight/Car Charges).
        # Same logic as Product Price; a clearly-named report-scoped column to bind so the SOP620 report never picks up a
        # plain SUM(extended_price) (e.g. Sales Amount) on freight rows.
        ('Product Price (Product)',"SUMX(FILTER('fact_sales_order_freight', NOT(TRIM('fact_sales_order_freight'[line_type]) = \"F\" || TRIM('fact_sales_order_freight'[line_type]) = \"FT\")), 'fact_sales_order_freight'[extended_price])", '\\$#,0.00', 'Price Adjustment'),
        ('Price Per Ton', "DIVIDE([Product Price], [Total Tons])", '\\$#,0.00', 'Price Adjustment'),
        # SOP620 bucket-report Price Per Ton — divides by the freight-excluded tons so freight rows show blank (0/blank)
        # instead of $0.00 (0/1). Product-line values identical to Price Per Ton. Bind alongside Total Tons (Product).
        ('Price Per Ton (Product)', "DIVIDE([Product Price (Product)], [Total Tons (Product)])", '\\$#,0.00', 'Price Adjustment'),

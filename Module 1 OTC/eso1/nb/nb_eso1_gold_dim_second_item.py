...
# ## nb_eso1_gold_dim_second_item
#
# **Gold `dim_second_item` processor** for Extended Sales Order 1 (Billable v Payable Freight).
# Builds ONE small dimension — `lh_jde_gold.rpt.dim_second_item` — the distinct sales-line second
# item number (SDLITM) plus ONE 'Included'/'Excluded' flag COLUMN per Ottawa variation, so a report
# slices/filters on that variation's flag column (fast) instead of a 145-value IN/NOT-IN filter or a
# slow per-row measure. Relates to the freight fact on second_item_number (1 -> many).
#
# Each variation's SDLITM rule is materialized as its own column (VARIATION_FILTERS). Two underlying
# item lists drive them: GROUND_ITEMS (145 "ground" SDLITM codes) and ASTM_ITEMS (3 ASTM codes):
#   • Ground variations         → item IN  GROUND_ITEMS   (these ARE the ground items)
#   • Whole Grain variations    → item NOT IN GROUND_ITEMS (whole grain = the complement)
#   • ASTM variation            → item IN  ASTM_ITEMS
# ('Included' = the item is KEPT by that variation's SDLITM filter.) The other per-variation filters
# (SDSRP3 PKG/BLK, SDMOT rail/truck, SDUOM, SDMCU, SDNXTR, SDCO) are already fact page-filter columns,
# NOT item-list logic — so they are NOT encoded here.
# Runs as an independent job (own table, own overwrite switch).
#
# ── BUILD (BATCH) ─────────────────────────────
#   • read the full F4211 (∪ F42119) snapshot, run build_dim() ONCE, overwrite the dim.

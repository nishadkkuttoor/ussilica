...
F41002   = "f41002_item_units_of_measure_conversion_factors"  # static — item-specific UOM->TN (Tier A)
F41003   = "f41003_unit_of_measure_standard_conversion"       # static — standard UOM->TN (Tier B)
...
FACT_VARIATION_COLS = [
    ...
    # ── standard UoM conversion — VOLUME (SDUOM->TN) + PRICE (SDUOM4->TN, F4211 rows only) ──
    "conversion_to_tons_rate",       # SDUOM->TN factor (TN->1.0, F41002, F41003, else 1.0)
    "quantity_shipped_tons",         # units_ordered (SDUORG) * conversion_to_tons_rate
    "missing_conversion_flag",       # 'Y' when the volume cascade resolves nothing
    "uom_pricing",                   # SDUOM4 (NULL on PO rows)
    "price_conversion_factor",       # SDUOM4->TN factor (NULL on PO rows)
    "converted_price_per_ton",       # unit_price / price factor on delivered F4211 lines
    "price_missing_conversion_flag", # 'Y' when the price cascade resolves nothing
]
...
def build_uom_cascades():
    ...
    i_fwd = (f41002.filter((F.trim("related_uom") == "TN") & (F.col("conversion_factor") != 0))
             .select(F.col("identifier_short_item").alias("itm"), F.trim("uom").alias("from_uom"),
                     F.col("conversion_factor").cast("double").alias("f")))
    i_rev = (f41002.filter((F.trim("uom") == "TN") & (F.col("conversion_factor") != 0))
    ...
    s_fwd = (f41003.filter((F.trim("related_uom") == "TN") & (F.col("conversion_factor") != 0))
             .select(F.trim("uom").alias("from_uom"), F.col("conversion_factor").cast("double").alias("f")))
    s_rev = (f41003.filter((F.trim("uom") == "TN") & (F.col("conversion_factor") != 0))
...
def build_fact():
    ...
    uom_item, uom_std = build_uom_cascades()  # SDUOM/SDUOM4 -> TN item (F41002) + standard (F41003) cascades
    ...
    # volume: TN->1.0, else item F41002, else standard F41003; *_raw (no 1.0 default) drives the flag
    ...
    # price: F4211 rows only (PO leg has no unit price); delivered = last<>980 AND next=999
    ...
    j = (base.alias("sd")
         ...
         # UOM->TN cascades: volume on SDUOM, price on SDUOM4 (item F41002 + standard F41003; deduped, no fan-out)
...
FACT_SOURCES = [
    ...
    {"silver": F41002,   "join": "static", "join_pairs": []},   # item UOM->TN cascade (Tier A)
    {"silver": F41003,   "join": "static", "join_pairs": []},   # standard UOM->TN cascade (Tier B)
]

...
# ## nb_eso1_gold_dim_order_number
#
# **Gold `dim_order_number` processor** for Extended Sales Order 1 (Billable v Payable Freight).
# Builds ONE small dimension — `lh_jde_gold.rpt.dim_order_number` — the distinct sales-order number
# (SDDOCO) plus one 'Included'/'Excluded' flag COLUMN per report that filters on a hand-picked order
# whitelist, so a report slices/filters on that report's flag column (fast) instead of an N-value
# order IN (...) filter. Relates to the freight fact on order_number (1 -> many).
#
# Each report's SDDOCO whitelist is materialized as its own column (ORDER_FILTERS). 'Included' = the
# order IS in that report's whitelist. Other per-report filters (line_type, status, LOB, etc.) are
# already fact page-filter columns, NOT encoded here.
#
# ⚠ An order-number whitelist is a SNAPSHOT (specific orders for a period), unlike a stable item list —
# refresh this dim (re-run with the updated list) whenever the report's whitelist changes.

...
# from the Silver sales-order-detail (F4211, ∪ F42119 history) and commission (F42005) Change
# Data Feed streams. Same conventions as nb_eso1_gold_fact_sales_order_freight (own table, own
# checkpoint root, own OVERWRITE, **no report filters — all filtering is Power BI page-level**;
# the query's INNER F4201/F0101 + ABAT1 band become page filters: sold_to IS NOT NULL,
...
# ── Silver sources ────────────────────────────────────────────────────────────
SILVER_SCHEMA    = "jde"             # schema `jde` (2026-07-26, was `jde_cdc`) — same as ESO4/ESO5; CDF required on every STREAMED source
...
def transform_fact(restrict_orders=None):
    ...
    sc  = load_silver_table(F42005_TBL)      # commission ledger — LEFT-joined (a sales line has 0..N commission records)
    ...
    # (restrict the DRIVER — F4211 lines — to the affected orders during CDC recompute.)
    if restrict_orders is not None:
        ln = ln.join(restrict_orders.alias("ro"),
                     (ln["company_key_order_no"] == F.col("ro.company_key_order_no")) &
                     (ln["order_type"]                == F.col("ro.order_type")) &
                     (ln["document_order_invoice_e"]  == F.col("ro.order_number")), "left_semi")
...
if not _exists(sname(F4211_TBL)):
    _errs.append(f"MISSING driver {sname(F4211_TBL)} — F4211 is the fact DRIVER; must be ingested to Silver with CDF enabled")
    print(f"  MISSING : {sname(F4211_TBL)}  (sales-order detail — the fact DRIVER)")
else:
    print(f"  OK      : {sname(F4211_TBL)}  (sales-order detail — the fact DRIVER)")
if not _exists(sname(F42005_TBL)):
    _errs.append(f"MISSING {sname(F42005_TBL)} — F42005 commission ledger must be ingested to Silver with CDF enabled")
    print(f"  MISSING : {sname(F42005_TBL)}  (commission ledger — LEFT-joined)")
else:
    print(f"  OK      : {sname(F42005_TBL)}  (commission ledger — LEFT-joined)")
if _errs:
    raise Exception("Commission-fact preflight FAILED: " + "; ".join(_errs)
                    + ". Build reused dims via old_nb (nb_dim_address_book, nb_dim_plant); "
                    + "ensure F4211 and F42005 are ingested to lh_jde_silver.jde with delta.enableChangeDataFeed=true.")
...
#   the freshly recomputed rows. F4211 is the DRIVER — a line change (incl. a purge from
#   F4211 to F42119, seen as a delete) recomputes the order's sales lines; an F42005 commission
#   change also recomputes the order so its lines pick up / drop the commission. Both are needed.
# =============================================================================

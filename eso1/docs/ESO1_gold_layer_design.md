...
5. **F4074 freight `ALAST` qualification** (open, 2026-07-27) — the master BvP query (`ESO1 Hubble Query.sql`) attaches F4074 **only** for `ALAST IN ('A03','FRTHIDE','FRTTAXN','FRTTAXY')` (value-qualification; ReportColumn14 = `SUM` over those). `build_fact()` instead collapses F4074 to **one** row per line via `row_number()` ordered by `price_adjustment_type` **across all adjustment types** (no whitelist), so `price_adjustment_type` / `freight_factor_value` / `adj_*` can reflect a non-freight adjustment Hubble would ignore, and multi-freight-adjustment lines store one value rather than the RC14 sum. **NOT fixed** because the same collapsed row also feeds the accounting `adj_gl_class` (SOP0007) / `adj_based_on_value` (SOP0006/0007/0008) / `adj_uom` (SOP0006/000x) / `adj_factor_value` (BP Freight) columns, which may deliberately read non-freight adjustment types — one row per line cannot serve both whitelists. **Resolution blocked** on confirming which `ALAST` set those SOP/BP variations require; if master BvP takes priority, restrict the F4074 candidate set to the freight whitelist before the pick (output changes on adjustment-bearing lines). Affects only lines with F4074 rows. See the 2026-07-27 query-vs-notebook verification.

SELECT
  ...
  SUM(F4211.SDPQOR) ReportColumn20,
...
FROM
  ...
  INNER JOIN PRODDTA.F4101 F4101 ON F4211.SDITM = F4101.IMITM
  LEFT JOIN PRODDTA.F4074 F4074 ON (
    (
      (
        (
          (F4074.ALDCTO = F4211.SDDCTO)
    ...
    FROM
      (
        ...
        SELECT
          ...
          END UMCNV1
        FROM
          PRODDTA.F41002 F41002_2
      ) F41002_3
    GROUP BY
      UMMCU,
      UMITM,
      UMUM
  ) FromConvTableF41002 ON (
    (FromConvTableF41002.UMMCU = N'            ')
  ...
  LEFT JOIN (
    ...
    GROUP BY
      ...
      UMITM,
      UMUM
  ) ToConvTableF41002 ON (
    (ToConvTableF41002.UMMCU = N'            ')
    ...
    FROM
      (
        ...
        SELECT
          ...
          END UMCNV1
        FROM
          PRODDTA.F41003 F41003_2
      ) F41003_3
    GROUP BY
      UMUM,
      UMRUM
  ) FromConvTableF41003 ON (F4211.SDUOM = FromConvTableF41003.UMUM)
  ...
  LEFT JOIN (
    ...
    GROUP BY
      UMUM,
      UMRUM
  ) ToConvTableF41003 ON (N'TN' = ToConvTableF41003.UMUM)
          ...
          (
            (
              (
                ...
                AND ((NOT (F4211.SDGLC IN (N'DZ01'))))
              )
            )
            AND ((NOT (F4211.SDLNTY IN (N'TL'))))
          )
        )
        AND ((F4211.SDDGL BETWEEN 126091 AND 126120))
      )
    )
    AND (
      (

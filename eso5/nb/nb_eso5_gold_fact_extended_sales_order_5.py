...
#          Hubble's orphan set = `po_holadd_superseded = 'N'`, which the report filters on.
...
#
# ── FILTERS: ZERO. NOT ONE. (user direction, 2026-07-20) ───────────────────────────────────────
...
#      aggregate, so it drops no row and an out-of-band facility gets a NULL rate, exactly as Hubble.
...
#
# ── AND NO STATUS FILTERING AT ALL (user direction, 2026-07-14) ────────────────────────────────
...
#          every OX HOLADD PO line is now a row, and Hubble's orphan set is `po_holadd_superseded='N'`.
...
#                                  AND po_holadd_superseded = 'N'      ← Hubble's NOT EXISTS, as a filter
...
#   of every SX/00750 sales line AND every purchase order line in the fact. See docs/ESO5_gold_layer_design.md §7b.
...
#
# ── BUILD (BATCH, architecture like ESO4's fact notebook) ──────────────────────────────────────
...
#
# JOINS (docx §4 "SQL Table Joins" — all LEFT/Outer):
...
# Declares each Silver source and how it relates to the F4211 spine (mirrors the ESO4 FACT_SOURCES
# structure). This fact's join graph is rich (SBXLOADPOVIEW/SBXLOADDETAIL/SBXUSSSAND legs) and applied

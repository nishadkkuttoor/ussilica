SELECT RawF0101.XSBXLOADPOVIEW_SDDOCO       SBXLOADPOVIEW_SDDOCO,
...
FROM   (SELECT DISTINCT SBXLOADPOVIEW.SDDOCO                                                                             XSBXLOADPOVIEW_SDDOCO,
                        ...
                        DBMS_UTILITY.GET_HASH_VALUE(TO_CHAR(F0101.ABAN8)
                                                     || N'_InDeX_0_SaLt', -1048576, 2097152)                             XRowIDX_F0101_0_XRowID,
                        DBMS_UTILITY.GET_HASH_VALUE(TO_CHAR(F0101.ABAN8)
                                                     || N'_InDeX_1_SaLt', -1048576, 2097152)                             XRowIDX_F0101_1_XRowID,
                        DBMS_UTILITY.GET_HASH_VALUE(TO_CHAR(F0101.ABAN8)
                                                     || N'_InDeX_2_SaLt', -1048576, 2097152)                             XRowIDX_F0101_2_XRowID
        FROM   (SELECT SDKCOO,
                       SDDOCO,
                       SDDCTO,
                       SDVR01,
                       ...
                       SDDSC1,
                       (select MAX(I.sddsc1)
                        from   proddta.F4211 I
                        where  I.sddoco = F4211.SDDOCO
                               and I.sddcto = F4211.SDDCTO
                               and I.sdlitm = 'BOL')                   AS BOL,
                       (select MAX(I.sddsc1)
                        from   proddta.F4211 I
                        where  I.sddoco = F4211.SDDOCO
                               and I.sddcto = F4211.SDDCTO
                               and I.sdlitm = 'SANDTKTNBR')            as SANDTKT,
                       F4211.SDUOM                                     AS UOM,
                       DECODE(F4211.SDPRP1, 'COM', ( F4211.SDUORG / 1000 ) / 2000,
                                            F4211.SDUORG / 1000)       AS Qty,
                       ...
                       (select MAX(pdnxtr)
                        ...
                        where  pddcto = 'OX'
                               ...
                               and pddoco = F4211.sddoco)              AS OXNXTR,
                       (select MAX(pdlttr)
                        from   proddta.F4311
                        where  pddcto = 'OX'
                               and pdkcoo = '00750'
                               and pdlitm = F4211.SDLITM
                               and pddoco = F4211.Sddoco)              AS OXLTTR,
                       CASE
                         WHEN F4211.SDLITM IN ( 'FRT' ) THEN (select SUM(pdAEXP)
                                                              from   proddta.F4311
                                                              where  pddcto = 'OX'
                                                                     and pdkcoo = '00750'
                                                                     and pdlitm = F4211.SDLITM
                        ...
                        where  gldct = 'OV'
                               ...
                               and gldoc in (select prdoc
                                             ...
                                             where  sdcars = pran8
                                                    ...
                                                    and prdct = 'OV')) GLPost,
                       (select MAX(to_char(to_date(to_char(1900000 + f43121.PRDGL), 'yyyyddd'), 'mm/dd/yyyy'))
                        from   proddta.F43121
                        where  F4211.sdcars = F43121.pran8
                               and F4211.sdkcoo = F43121.prkcoo
                               and F4211.sddoco = F43121.prdoco
                               and F4211.sdlitm = F43121.prlitm
                               and F43121.prdct = 'OV'
                               AND F43121.PRMATC = '1'
                               AND F43121.PRDGL > 1)                   GLdgj,
                       SDLNID
                FROM   PRODDTA.F4211
                WHERE  F4211.SDDCTO = 'SX'
                       and F4211.sdlnty <> 'TL'
                       and F4211.sdkcoo = '00750'
                       and F4211.SDLITM <> 'HOLADD') SBXLOADPOVIEW
               LEFT JOIN PRODDTA.F554201T F554201T
                 ON ( ( SUBSTR(N'00000'
                                || TO_CHAR(RTRIM(LTRIM(SBXLOADPOVIEW.SDKCOO))), LENGTH(TO_CHAR(RTRIM(LTRIM(SBXLOADPOVIEW.SDKCOO)))) + 1, 5) = F554201T.QCKCOO )
                      AND ( SBXLOADPOVIEW.SDDOCO = F554201T.QCDOCO ) )
                    AND ( SBXLOADPOVIEW.SDDCTO = F554201T.QCDCTO )
               LEFT JOIN (SELECT F0101.ABALPH,
                                 F0101.ABURAT,
                                 F0101.ABAN8
                          FROM   PRODDTA.F0101 F0101
                          WHERE  ( ( F0101.ABAT1 BETWEEN N'A  ' AND N'P  ' )
                                    OR ( F0101.ABAT1 BETWEEN N'R  ' AND N'ZZZ' ) )) F0101
                 ON SBXLOADPOVIEW.LOFA = F0101.ABAN8
        WHERE  ( (( ( TO_CHAR(SBXLOADPOVIEW.ORDATE, N'YYYY') - 1900 ) * 1000 + TO_CHAR(SBXLOADPOVIEW.ORDATE, N'DDD') BETWEEN 126224 AND 126224 ))
                 AND ( SBXLOADPOVIEW.SDLITM = N'FRT                      ' ) )
               AND ( SBXLOADPOVIEW.SDNXTR < N'581' )) RawF0101
GROUP  BY RawF0101.XSBXLOADPOVIEW_SDDOCO,
...
ORDER  BY SBXLOADPOVIEW_ORDATE ASC,

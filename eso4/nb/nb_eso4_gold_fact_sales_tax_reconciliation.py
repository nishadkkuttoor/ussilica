...
# reconciliation; mirrors Hubble TX002). Dims are built by separate notebooks.
...
#
# JOINS  (Extended Sales Order 4.docx §4; NO fact-row filters)
# ─────────────────────────────────────────────────────────────────────────────
#   Source        Role             Join
#   F4211         Spine            (spine)
...
# GRAIN
# ─────────────────────────────────────────────────────────────────────────────
# One row per Hubble outer GROUP BY tuple: the inner SELECT DISTINCT (incl. the F03B11 PK)
...
# integer amounts; Silver is already decoded, so the placeholder is 1.0 (carried for lineage).
SHIFT_FACTOR = 1.0
...
# 2) FACT BUILDER
# ----------------------------------------------------------------------------
# NOTE — why this differs structurally from nb_silver_to_gold_eso7_v2_fact.py:
# ESO4 is PRE-AGGREGATED to the Hubble outer GROUP BY grain (one row per display tuple, the four
...
# FACT_BUSINESS_COLS drives the final .select). ESO7's fact is ATOMIC (order-line grain): build_fact()
# returns proj.distinct() with NO GROUP BY / SUM — measures aggregate in DAX at report time — so it
# has only a 4-column LINE_KEYS identity and no group/measure/business column lists. (This
# report-shaped grain is gate-review Finding 1 — see eso4/docs/ESO4_Gate_Review.docx.)
#
...
def build_fact():
    ...
    sel = j.select(
        ...
        # NOTE (2026-07-26): the JDE numeric identifiers below are NOT cast — Silver already stores them as
        # int64 (data-owner confirmed), so an explicit .cast("long") is unnecessary and the columns match the
        # TMDL int64 (and the int64 dim_address_* PKs) natively. This also matches the old fab_25-07-2026
        # notebook. (avalara_code still casts internally; document_company / document_type stay STRING so
        # JDE leading zeros survive.)
...
# 3) FACT SOURCES
# ----------------------------------------------------------------------------
# Declares each Silver source and how it joins to the F4211 spine (mirrors the ESO7
# FACT_SOURCES structure). build_fact() applies these joins directly — there is no CDF
# affected-lines machinery here (batch build); the RUN preflight below uses this list to
# confirm every source exists before the build. join_pairs: (source_col, f4211_col) tuples.
FACT_SOURCES = [
